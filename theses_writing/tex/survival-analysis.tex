...
In this extrension, the occurrence of the event of interest is represented through a
collection of competing causes. Let $N$ be a random variable denoting the number of
competing causes related to the occurrence of an interesting event with the following
...
associated with these causes, assumed to be independent random variables with
proper survival function $S(t)$ and does not depend of N. In order to include those
individuals that are not susceptible to the event occurrence, the observed event time
is then defined as the minimum among the latent times,
\begin{equation}
\label{eq:latent_event_time}
T=\min\{Z_1,\ldots,Z_N\},
\end{equation}
with $P\left(Z_{0} = \infty\right) = 1$.

Given the previous definitions, let $\{a_n\}$ be a real number sequence and
$s \in \left[0,1\right]$. Therefore, the generating function \cite{feller1971introduction} $A\left(s\right)$ of the
sequence $\{a_n\}$ is given by
\begin{equation}
A\left(s\right) = a_{0} + a_{1} s + a_{2} s^2 + \ldots \text{.}
\end{equation}

The survival function of the random variable $T$ is then obtained by,
\begin{eqnarray}
\label{eq:long_term_survival}
S_{p}(t) &=& P[N = 0] + P[Z_1 > t,Z_2 > t, ...,Z_N > t,N \geq 1] \nonumber \\
...
p_0.
\end{equation}
Thus, $p_0$ is precisely the long-term survival probability and provides a direct
interpretation of the proportion of individuals who are not susceptible to the event of
interest.
