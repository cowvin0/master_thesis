...
\begin{table}[htb]
    \centering
    \caption{Complete-data log-likelihood contributions for the different
    distributions considered for the latent number of competing causes $N$.
    See proof in appendix (ADD THE APPENDIX HERE LATER!!!!!!)}
    \label{tab:complete_loglikelihood}
    \small
    \renewcommand{\arraystretch}{1.4}
    \begin{tabular}{p{0.18\textwidth} p{0.74\textwidth}}
        \hline
        Distribution of $N$ & Complete-data log-likelihood $\ell_c$ \\
        \hline
        ...
        \sum_{i=1}^n
        \Bigg[
        \log\Gamma\left(\alpha^{-1}+n_i\right)
...
\right]^{-1/\alpha},
\end{equation}
onde \(\theta_i>0\) representa o número médio de riscos latentes e \(\alpha\) é um parâmetro de dispersão.
...
\mathrm{Var}(N_i)
=
\theta_i+\alpha\theta_i^2.
\end{equation}

Assim, valores positivos de \(\alpha\) produzem sobredispersão em relação ao modelo de Poisson. Além disso,
essa parametrização engloba importantes casos particulares. Quando \(\alpha \to 0\), recupera-se a
distribuição de Poisson e, consequentemente, o modelo GG-KM apresentado na seção anterior. Já o caso
\(\alpha=-1\) conduz ao modelo de mistura de \citeonline{berkson1952survival}. Neste trabalho, entretanto,
considera-se apenas o cenário \(\alpha>0\), correspondente à presença de sobredispersão.
...
Quando o parâmetro de dispersão \(\alpha\) é previamente especificado, ele é mantido fixo ao
longo do processo iterativo e apenas os parâmetros \(\boldsymbol{\alpha}\), \(a\), \(d\) e \(p\) são estimados.
Por outro lado, quando \(\alpha\) não é previamente informado, sua estimação é realizada conjuntamente com os
...
algoritmo decorre da presença do parâmetro de dispersão \(\alpha\), cuja
atualização não admite solução analítica em forma fechada. De fato,
\(\alpha\) aparece simultaneamente nos termos \(\log(1+\alpha\theta_iF_{GG}(t_i))\), \(1/\alpha\) e nas
...
e \((a,d,p)\) são atualizados por meio da maximização da função auxiliar, enquanto \(\alpha\) é atualizado
...
% parâmetro de dispersão \(\alpha\), cuja atualização a partir da função auxiliar não conduz a uma etapa de
% maximização simples. Assim, os parâmetros \(\boldsymbol{\alpha}\) e \((a,d,p)\) são atualizados por meio da
% maximização da função auxiliar, enquanto \(\alpha\) é atualizado diretamente pela maximização da
...
Quando \(\alpha\) não é previamente especificado, realiza-se adicionalmente a etapa ECME
