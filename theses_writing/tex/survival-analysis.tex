...
individuals are commonly referred to as \emph{cured} or \emph{immune} with respect to
the event under consideration \cite{maller1996survival}. The presence of such individuals gives
rise to the class of models commonly referred to as \emph{long-term survival models} or
\emph{cure-rate models}.
...
extension of the framework originally proposed by \citeonline{chen1999new} and
developed in a general form by \citeonline{rodrigues2009unification}. In this framework,
the occurrence of the event of interest is represented through a collection of latent
competing causes. Let $N$ denote the unobserved number of latent causes associated with
the occurrence of the event, with probability mass function
\begin{equation}
...
associated with these causes, assumed to be independent and identically distributed with
proper survival function $S(t)$. The observed event time is then defined as the minimum
among the latent times,
\begin{equation}
\label{eq:latent_event_time}
Y=\min\{Z_1,\ldots,Z_N\},
\end{equation}
with the convention that $Y=\infty$ when $N=0$.

The variables $N$ and $Z_1,\ldots,Z_N$ are latent and therefore unobserved; only the
resulting event time $Y$ is observed, subject to censoring. The case $N=0$ plays a
fundamental role in the long-term survival framework. When $N=0$, no latent cause
capable of producing the event is present, and the individual is consequently not
susceptible to the event. Thus, the probability $p_0=P(N=0)$ represents the long-term
proportion of cured or immune individuals in the population.

The survival function of $Y$ is obtained by conditioning on the unobserved number of competing causes. Specifically,
\begin{equation}
...
P(Y>t)
=
\sum_{n=0}^{\infty}p_n\{S(t)\}^n.
\end{equation}
Therefore, if $A(s)=\sum_{n=0}^{\infty}p_ns^n$ denotes the probability generating
function of $N$, the long-term survival function can be written compactly as
\begin{equation}
\label{eq:long_term_survival_pgf}
S_p(t)=A\{S(t)\}.
\end{equation}

An important consequence of this formulation is obtained by considering the limit as
$t\rightarrow\infty$. Since $S(t)$ is a proper survival function, $\lim_{t\rightarrow\infty}S(t)=0$,
and hence
\begin{equation}
\label{eq:long_term_limit}
\lim_{t\rightarrow\infty}S_p(t)
=
A(0)
=
p_0.
\end{equation}
Thus, $p_0$ is precisely the long-term survival probability and provides a direct interpretation of the proportion of individuals who are not susceptible to the event of interest.
