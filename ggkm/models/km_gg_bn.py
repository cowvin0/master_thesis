...
class GG_KM(KernelFunc):
    ...
    def _phi_gradient(
        ...
        grad = np.sum(
            np.log1p(phi * A) / phi**2 - (1.0 / phi + delta) * A / (1.0 + phi * A)
        )
    ...
    def _alpha_objective(self, alpha, Nhat):
        ...
        Q -= (self.lambda_reg / 2) * (alpha @ self.K_ @ alpha)
    ...
    def _alpha_gradient(self, alpha, Nhat):
        ...
        grad = -self.K_ @ score + self.lambda_reg * self.K_ @ alpha
    ...
    def fit(
        ...
        while True:
            ...
            if self.estimate_phi:
                result_phi = minimize(
                    fun=self._phi_objective,
                    x0=np.array([self.phi]),
                    jac=self._phi_gradient,
                    args=(alpha, t, delta),
                    method="L-BFGS-B",
                    bounds=[(1e-8, None)],
                )
