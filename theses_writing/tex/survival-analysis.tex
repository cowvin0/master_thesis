...
knowledge of the exact time at which it occurred. Each type of censoring can be observe in Figure \ref{fig:censoring_system}.
...
Therefore, survival data are generally composed of both complete and censored observations. This characteristic
distinguishes survival analysis from many other areas of statistics \cite{aalen2008survival}, since the
exact event time is not always observed. As a result, specialized statistical methods are required to
properly account for censored observations when modeling time-to-event outcomes. Notice also that, although
the examples of censoring were given in the context of a credit union, censoring can occur in many
other scenarios. Given the definition of censoring in this section, the next section will introduce
the survival function and hazard rate for

\section{Survival function and hazard rate}
