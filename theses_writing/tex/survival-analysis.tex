...
The methods developed in this work consider different distributions for the latent
variable $N$, representing the number of competing causes. Specifically, the Bernoulli,
Binomial, Negative Binomial, and Poisson distributions are considered. The corresponding
results are presented in Table (ADD TABLE REFERENCE HERE), and their derivations are
given below, beginning with the Bernoulli distribution.
...
\end{equation}
where $f(t)=-S'(t)$ is the density associated with the proper survival function $S(t)$.
...
S_p(t)
=
\left[1+\alpha\theta\{1-S(t)\}\right]^{-1/\alpha}.
\end{equation}
...
E[N]=\theta
\qquad\text{and}\qquad
\operatorname{Var}(N)=\theta+\alpha\theta.
\]
...
among the Binomial, Poisson, and Negative Binomial distributions. Specifically, when
\[
\alpha=-\frac{1}{K},
\]
...
=
\theta f(t)
\left[1+\alpha\theta\{1-S(t)\}\right]^{-1/\alpha-1},
\end{equation}
...
parameter $\theta>0$, i.e.,
\[
N\sim\mathrm{Poisson}(\theta).
\]
...
\begin{equation}
\label{eq:poisson_pgf}
A(s)
=
\exp\{\theta(s-1)\}.
\end{equation}

Therefore, the corresponding long-term survival function is
\begin{equation}
...
A\left(S(t)\right)
=
\exp\left\{-\theta[1-S(t)]\right\}.
\end{equation}
Since $F(t)=1-S(t)$, this can equivalently be written as
\begin{equation}
S_p(t)
=
\exp\{-\theta F(t)\}.
...
\begin{equation}
\label{eq:poisson_cure_fraction}
p_0
=
\lim_{t\rightarrow\infty}S_p(t)
=
...
\begin{equation}
\label{eq:poisson_density}
f_p(t)
=
\theta f(t)
\exp\{-\theta F(t)\},
\end{equation}
and the associated hazard rate function is
\begin{equation}
\label{eq:poisson_hazard}
h_p(t)
=
\theta f(t).
\end{equation}

The Poisson specification is particularly important in long-term survival analysis
because it leads to the promotion-time cure model \cite{yakovlev1996stochastic}. Thus,
the Poisson distribution provides a direct connection between the general latent
competing-cause formulation and the promotion-time approach to cure-rate modeling.
