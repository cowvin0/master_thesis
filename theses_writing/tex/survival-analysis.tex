...
According to \citeonline{moore2016applied}, survival analysis methods depend on the survival distribution,
and the two main ways of specifying it are the survival function and the hazard rate function. The survival
function gives the expected proportion of individuals for which the event has not yet happened by time $t$.
Formally, let $T$ denotes the survival time, the survival function is given by:
\begin{equation}
\label{eq:surv_function}
S\left(t\right) = P\left(T>t\right)\text{, } t>0\text{, } 0 < S\left(t\right) < 1
\end{equation}

% Although the name survival function resembles an event of death, it can be used in a quite general sense,
% that is, it does not need to be related to the study of death.
